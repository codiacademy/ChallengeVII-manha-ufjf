...
const ChartPreview = ({
  ...
  const handleExportChart = async () => {
    ...
    try {
      ...
      alert(
        "Ocorreu um erro ao exportar o gráfico. Por favor, tente novamente."
      );
    }
  };
      ...
      return (
        <div className="flex items-center justify-center h-full border border-dashed border-gray-300 rounded-lg">
          <p className="text-gray-400">
            Dados não disponíveis ou configuração incompleta
          </p>
        </div>
      );
    }
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md ${isModal ? "h-full" : "p-4"} ${
        containerClassName || ""
      }`}
    >
      <div
        className={`flex justify-between items-center ${
          isModal ? "p-4 pb-2" : "mb-4"
        }`}
      >
        <h3 className="text-lg font-semibold text-[#a243d2]">
          {defaultConfig.title}
        </h3>
        {showExport && (
          <button
