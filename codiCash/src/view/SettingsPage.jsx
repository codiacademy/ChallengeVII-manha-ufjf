...
// Componentes reutilizáveis fora do componente principal
      ...
      >
        ...
        {options.map((option) => (
          <option key={option.id} value={option.id}>
            {option.tipo ||
              option.nome ||
              `${option.nome} - ${option.localizacao}`}
          </option>
        ))}
      </select>
    </div>
        ...
        {data.map((item) => (
          <tr key={item.id}>
            {columns.map((col) => (
              <td
                key={col.key}
                className="px-3 py-2 sm:px-6 sm:py-4 whitespace-nowrap"
              >
...
// Componentes de Modal fora do componente principal para evitar re-render
const ClientModal = memo(
  ({ isOpen, onClose, forms, handleFormChange, handleSubmit, currentItem }) => (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      position="center"
    ...
    >
      <h2 className="text-lg sm:text-xl font-semibold mb-4 text-[#a243d2]">
        {currentItem ? "Editar Cliente" : "Adicionar Novo Cliente"}
      </h2>
      <form onSubmit={handleSubmit}>
        <FormInput
          label="Nome"
          name="nome"
          value={forms.client.nome}
        ...
        <FormInput
          label="Telefone"
          name="telefone"
          value={forms.client.telefone}
          onChange={(e) => handleFormChange("client", e)}
        />
        <FormButtons
          onCancel={onClose}
          onSubmitText={currentItem ? "Atualizar" : "Salvar"}
        />
      </form>
    </Modal>
  )
...
const CursoModal = memo(
  ({
    ...
    >
      <h2 className="text-lg sm:text-xl font-semibold mb-4 text-[#a243d2]">
        {currentItem ? "Editar Curso" : "Adicionar Novo Curso"}
      </h2>
      <form onSubmit={handleSubmit}>
        <FormInput
          label="Nome"
          name="nome"
          value={forms.curso.nome}
...
const FilialModal = memo(
  ({ isOpen, onClose, forms, handleFormChange, handleSubmit, currentItem }) => (
    ...
    >
      <h2 className="text-lg sm:text-xl font-semibold mb-4 text-[#a243d2]">
        {currentItem ? "Editar Filial" : "Adicionar Nova Filial"}
      </h2>
      <form onSubmit={handleSubmit}>
        <FormInput
          label="Nome"
          name="nome"
          value={forms.filial.nome}
          onChange={(e) => handleFormChange("filial", e)}
        />
        <FormInput
          label="Localização"
          name="localizacao"
          value={forms.filial.localizacao}
          onChange={(e) => handleFormChange("filial", e)}
        />
        <FormButtons
          onCancel={onClose}
...
const UserModal = memo(
  ({ isOpen, onClose, forms, handleFormChange, handleSubmit, currentItem }) => (
    ...
    >
      <h2 className="text-lg sm:text-xl font-semibold mb-4 text-[#a243d2]">
        {currentItem ? "Editar Usuário" : "Adicionar Novo Usuário"}
      </h2>
      <form onSubmit={handleSubmit}>
        <FormInput
          label="Nome"
          name="nome"
          value={forms.user.nome}
        ...
        <FormInput
          ...
          placeholder={
            currentItem ? "Deixe em branco para manter a senha atual" : ""
          }
        />
        <FormSelect
          label="Papel"
...
const VendedorModal = memo(
  ({
    ...
    >
      <h2 className="text-lg sm:text-xl font-semibold mb-4 text-[#a243d2]">
        {currentItem ? "Editar Vendedor" : "Adicionar Novo Vendedor"}
      </h2>
      <form onSubmit={handleSubmit}>
        <FormInput
          label="Nome"
          name="nome"
          value={forms.vendedor.nome}
...
const SettingsPage = () => {
  ...
  const fetchFiliais = useCallback(
    () => fetchData("filiais", setFiliais),
    [fetchData]
  );
  const fetchModalidades = useCallback(
    () => fetchData("modalidades", setModalidades),
    [fetchData]
  );
  ...
  const getModalidadeName = useCallback(
    (modalidadeId) => {
  ...
  const handleSubmit = useCallback(
    async (e, endpoint, formName, currentItemName, successMessage) => {
      e.preventDefault();
      ...
      try {
        ...
        fetchData(
          endpoint,
          eval(`set${endpoint.charAt(0).toUpperCase() + endpoint.slice(1)}`)
        );
        toggleModal(formName, false);

        // Não resetar o formulário completamente
        if (!currentItem) {
          setForms((prev) => ({
            ...prev,
            [formName]: Object.fromEntries(
              Object.keys(prev[formName]).map((key) => [
                key,
                key === "papel" ? "user" : "",
              ])
            ),
  ...
  const handleDelete = useCallback(
    async (
      ...
      if (
        window.confirm(`Tem certeza que deseja deletar este ${successMessage}?`)
      ) {
        try {
  ...
  const handleUpdateProfile = useCallback(
    async (e) => {
      e.preventDefault();
      const { novaSenha, confirmarSenha, senhaAtual, ...profileData } =
        forms.profile;
  ...
  return (
    <div className="p-4 sm:p-6">
      <h1 className="text-2xl sm:text-3xl font-bold text-[#a243d2] mb-4 sm:mb-6">
        Configurações
      </h1>
        ...
        <div>
          <SectionHeader
            ...
            onAdd={() => {
              setForms((prev) => ({
                ...prev,
                curso: { nome: "", descricao: "", preco: "", modalidadeId: "" },
              }));
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                key: "modalidadeId",
                title: "Modalidade",
                render: (item) => getModalidadeName(item.modalidadeId),
              },
            ]}
      ...
      {activeTab === "profile" && (
        <div className="bg-white rounded-lg shadow p-4 sm:p-6">
          ...
          <form onSubmit={handleUpdateProfile}>
            ...
            <h3 className="text-md sm:text-lg font-medium mb-3 sm:mb-4">
              Alterar Senha
            </h3>
            <div className="grid grid-cols-1 gap-4 sm:gap-6 mb-4 sm:mb-6">
              <FormInput
        ...
        <div>
          <SectionHeader
            ...
            onAdd={() => {
              setForms((prev) => ({
                ...prev,
                user: { nome: "", email: "", senha: "", papel: "user" },
              }));
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: "email", title: "Email" },
              {
                key: "papel",
                title: "Papel",
                render: (item) => item.papel.toLowerCase(),
              },
            ]}
            onEdit={(userItem) => handleEdit(userItem, "user", "user")}
            onDelete={(id) =>
              handleDelete(id, "usuarios", "Usuário", true, id === user?.id)
            }
          />
        </div>
      )}
          ...
          <DataTable
            ...
            columns={[
              { key: "nome", title: "Nome" },
              {
                key: "filialId",
                title: "Filial",
                render: (item) => getFilialName(item.filialId),
              },
            ]}
      ...
      {/* Renderizar todos os modais com props específicas */}
      <ClientModal
        ...
        handleSubmit={(e) =>
          handleSubmit(e, "clientes", "client", "client", "Cliente")
        }
        currentItem={currentItems.client}
      />
      ...
      <CursoModal
        ...
        handleFormChange={handleFormChange}
        handleSubmit={(e) =>
          handleSubmit(e, "cursos", "curso", "curso", "Curso")
        }
        currentItem={currentItems.curso}
      ...
      <FilialModal
        ...
        handleFormChange={handleFormChange}
        handleSubmit={(e) =>
          handleSubmit(e, "filiais", "filial", "filial", "Filial")
        }
        currentItem={currentItems.filial}
      />
      ...
      <UserModal
        ...
        handleFormChange={handleFormChange}
        handleSubmit={(e) =>
          handleSubmit(e, "usuarios", "user", "user", "Usuário")
        }
        currentItem={currentItems.user}
      />
      ...
      <VendedorModal
        ...
        handleFormChange={handleFormChange}
        handleSubmit={(e) =>
          handleSubmit(e, "vendedores", "vendedor", "vendedor", "Vendedor")
        }
        currentItem={currentItems.vendedor}
        filiais={filiais}
      />
    </div>
  );
